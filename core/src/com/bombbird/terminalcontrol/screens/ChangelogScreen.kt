...
class ChangelogScreen(game: TerminalControl, background: Image?) : StandardUIScreen(game, background) {
    companion object {
        val changeLogContent = LinkedHashMap<String, LinkedHashMap<String, Array<String>>>()
        ...
        var latestMajor: String? = null
        var latestMinor: String? = null
...
//            content30.add("-Fixed crash when hold is selected when aircraft has no route waypoints")
            ...
            if (full) content25.add("-Datatag: Added customisable data tags: go to Settings => Data tag => Datatag style => Manage layouts...")
            ...
            if (android) {
                if (!full) content22.add("-Airports: More airports have been made available - complete surveys or watch ads to unlock new airports for a set period of time!")
            ...
            if (desktop) content21.add("-TTS: Added TTS for desktop (Linux, Windows XP and later); if you are using Windows, you can install more voices from Microsoft")
            content21.add("-Bug fixes, optimisations")
            addVersionInfo("1.5", "2101.1", content21)

            //Version 1.5.2014.2
            val content20 = Array<String>()
            if (full) content20.add("-Mechanics: Added circling approaches for rwy 14R at TCOO and rwy 27 at TCBE")
            ...
            if (android) content16.add("-UI: Added distance measuring tool - switch to \"Dist mode\" and measure the distance between 2 points with 2 fingers")
            if (desktop) content16.add("-UI: Added distance measuring tool - right click and drag to measure distance between 2 points")
            ...
            if (full) content15.add("-TCMD, TCPG: Updated MVA data")
            ...
            if (full) content14.add("-TCTT: Added new south runway configurations, SIDs and STARs")
            ...
            if (full) content10.add("-TCPG: Added STAR transitions for TCPG, TCPO")
    ...
    private fun loadContent() {
        loadHashmapContent(TerminalControl.full, Gdx.app.type == Application.ApplicationType.Android, Gdx.app.type == Application.ApplicationType.Desktop)

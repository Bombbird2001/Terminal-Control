    ...
    public void update() {
        ...
        if (radarScreen.aircrafts.get("EVA226") != null) {
            ...
            if (prompt5 && aircraft.getControlState() == 0) {
                prompt5 = false;
                tutorialMsg("Alright! The aircraft has been handed off to the tower controller. For every departure that gets handed over, or for every arrival that lands, you receive 1 point to add to your score. But for every separation infringement, your score will be halved. Be careful!");
                timer.scheduleTask(new Timer.Task() {
                    @Override
                    public void run() {

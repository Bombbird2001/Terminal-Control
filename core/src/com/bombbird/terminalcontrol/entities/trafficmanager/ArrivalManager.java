    ...
    public static void checkArrival(Arrival arrival) {
        for (Aircraft aircraft : TerminalControl.radarScreen.aircrafts.values()) {
            if (aircraft instanceof Arrival) {
                if (arrival.getAltitude() - aircraft.getAltitude() < 2500 && MathTools.pixelToNm(MathTools.distanceBetween(arrival.getX(), arrival.getY(), aircraft.getX(), aircraft.getY())) < 6) {
